...
int main()
{
    ...
    CHECK(themeMenuBg(THEME_DARK) == (int)UiTheme::palette(UiTheme::PALETTE_DARK).chrome, "dark chrome comes from the palette");
    CHECK(themeAccent(THEME_DARK) == (int)0xffe8a33d, "dark theme uses the amber accent");
    ...
    CHECK(themeMenuBg(THEME_LIGHT) != themeMenuBg(THEME_DARK), "light theme differs");
    CHECK(clampTheme(THEME_LIGHT) == THEME_LIGHT, "light theme kept");
    CHECK(themeWantsDarkCanvas(THEME_LIGHT) == 0 && themeWantsDarkCanvas(THEME_DARK) == 1,
        "light theme pairs with the paper canvas");
